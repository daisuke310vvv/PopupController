...
open class PopupController: UIViewController {
    ...
    public enum PopupLayout {
        case top, center, bottom, twitterSharePopupCenter(keyBoardHeight: CGFloat)
        ...
        func origin(_ view: UIView, size: CGSize = UIScreen.main.bounds.size) -> CGPoint {
            ...
            case .twitterSharePopupCenter(let keyBoardHeight): return CGPoint(x: (size.width - view.frame.width) / 2, y: (size.height - keyBoardHeight) / 2 - view.frame.height)
            }
        }
    }
